...
from ..data.utils import _makedirs, safe_divide, save_img
...
def validation_eval(run_path, model, options, generator):
    ...
    for sample_index, (batch_x, batch_y, all_batch_x, batch_y_mask, file_ind) \
        ...
        file_name = "ground_truth"+ os.sep + "ground_truth_{0}.png".format(sample_index)

        file_path = join(predictions_path, file_name)
        # print ("save display_y to {0}".format(file_path))

        plot_img(subplot_index, display_y, 'Ground Truth', is_rgb=True)

        plt.savefig(file_path, bbox_inches='tight', format='png', dpi=300)

        # original

        file_name = "original"+ os.sep + "original_{0}.png".format(sample_index)

        file_path = join(predictions_path, file_name)

        plot_img(subplot_index, display_all_x, 'Original image', is_rgb=True)

        plt.savefig(file_path, bbox_inches='tight', format='png', dpi=300)

        # prediction

        file_name = "prediction"+ os.sep + "prediction_{0}.png".format(sample_index)

        file_path = join(predictions_path, file_name)

        plot_img(subplot_index, display_pred, 'Prediction', is_rgb=True)
